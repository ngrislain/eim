...
namespace json {
	...
	public:
		...
		S& get(std::string key) {return *(dynamic_cast<S*>((*this)[key].get()));};
		template <typename S, typename = if_serializable<S>>
		Object& set(std::string key, S *s) {
			(*this)[key] = std::unique_ptr<S>(s);
			return *this;
		};
		template <typename N, typename = if_arithmetic<N>>
		Object& set(std::string key, N n) {return set(key, new json::Number(n));}
		Object& set(std::string key, const std::string &s) {return set(key, new json::String(s));}
		template <typename S, typename = if_serializable<S>>
	...
	class Array : public std::vector<std::unique_ptr<Serializable>>, public Serializable {
	public:
		virtual std::ostream& json(std::ostream& os, int indent=0) const override;
		template <typename S, typename = if_serializable<S>>
		S& get() {return *(dynamic_cast<S*>(back().get()));};
		template <typename S, typename = if_serializable<S>>
		S& get(int i) {return *(dynamic_cast<S*>((*this)[i].get()));};
		template <typename S, typename = if_serializable<S>>
		Array& add(S *s) {
		...
		template <typename N, typename = if_arithmetic<N>>
		Array& add(N n) {return add(new Number(n));}
		Array& add(const std::string &s) {return add(new String(s));}
		template <typename S, typename = if_serializable<S>>
		Array& add() {
